...
template <typename T>
concept PriceLevelLookup = requires(T lookup, const T clookup, Price price, PriceLevel* level) {
    ...
    { clookup.min() }  -> std::same_as<PriceLevel*>; // nullptr if empty
    { clookup.max() }  -> std::same_as<PriceLevel*>; // nullptr if empty
